...
function stopSliding(slider){
    var slideCurrent = document.getElementById("slider"+slider);
    var slideAbove = document.getElementById("slider"+slider+1);
    if(slider==1){
    ...
    var width = parseInt(window.getComputedStyle(slideCurrent).getPropertyValue("width"));
    ...
    if(difference>0){
        ...
        slideCurrent.style.left = left.toString().concat("px");
    }
    var offset = (width - absDifference).toString().concat("px");
    slideCurrent.style.width = offset;
